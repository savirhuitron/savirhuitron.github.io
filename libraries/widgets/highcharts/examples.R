...
hPlot(freq ~ Exer, data = plyr::count(MASS::survey, c('Sex', 'Exer')), type = 'column', group = 'Sex')


b <- hPlot(VOLUMEN ~ TIEMPO, data = op  , type = 'column', group = 'MERCADO', title = "VOLUMEN", subtitle = "cifras en miles")
b$xAxis(title = 'AÑOS')
b$yAxis(title = 'AÑOS')
b

op <- read.csv("op_ejemplo.csv")



op$VOLUMEN <- round(op$VOLUMEN,0)

head(ej1)

ej2 <- plyr::count(MASS::survey, c('Sex', 'Exer'))
...
prueba <- readFormat (hexViewFile("dwh_adm.insumos.dmp"), memFormat(int1 = integer4))
prueba <- read.dbf("dwh_adm.insumos.dbf")
